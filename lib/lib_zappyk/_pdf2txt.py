...
###############################################################################
def _pdf2txt(file_name, set_debug=def_debug):
    debug = set_debug
    ...
    for page in PDFPage.create_pages(document):
        ...
        lwc = __words_coord(__parse_obj(layout._objs))

        if out_sort:
            lwc = __words_coord_list2dict(lwc)
            ...
            for i in range(len(lwc)):
                ...
                #CZ#b + =a[i][j].encode(word_encode) + " "
                    if (j == 0):
                        owc += str(awc[i][j].encode(word_encode)) + " =>| "
                    else:
                        owc += str(awc[i][j]) + " | "
                '''
                ...
                owc = ("pag.norm=|%5s| %7s | %7s | %7s | %7s |=[%s]" % (num_pages, lwc[i][1], lwc[i][2], lwc[i][3], lwc[i][4], lwc[i][0].encode(word_encode)))
                if debug >= 1:
                    print("#%s" % owc)
...
def __parse_obj(lt_objs):
    ...
    for obj in lt_objs:
        ...
        # if it's a container, recurse
            ...
            __parse_obj(obj._objs)
    return(words)

###############################################################################
def __get_ymax(coord):
...
def __words_coord(words):
    ...
    #CZ#if (a!=" " and a!="\n" and a!='!' and a!="?" and a!='.' and a!="," and a!="(" and a!=")" and a!=":" and a!=" -" and a!=";"):
        if (a != " " and a != "\n"):
            ...
            if (word == ""):
                y = []
                x = __get_xmin(words[i][0])
                y.append(__get_ymax(words[i][0]))
            else:
                y.append(__get_ymax(words[i][0]))
            word += words[i][1]
        else:
            ...
            if out_version == 1:
                if (word != ""):
                    words_fin.append([word, x, __get_ymin(words[i-1][0]), __get_xmax(words[i-1][0]), max(y)])
                #CZ#word = ""
            ...
            #___________________________________________________________________________________________________________
                ...
                if not (cnt_space > 0 and cnt_space <= max_space):
                    if (word != ""):
                        words_fin.append([word, x, __get_ymin(words[i - 1][0]), __get_xmax(words[i - 1][0]), __max(y)])
                        word = ""

    return(words_fin)

###############################################################################
def __words_coord_list2dict(words):
    elements = []
    for i in range(len(words)):
        element = {}
        element['word'] = str(words[i][0].encode(word_encode))
