...
#CZ#_os._print_command(command)
    ...
    #CZ#command_exit = command_proc.wait()
        ...
        (command_exit
        ,command_stdo
        ,command_stde) = _popen_normalize(command_exit, command_stdo, command_stde)
...
def _command(command_exec=None):
    ...
    try:
    #CZ#(return_code) = _system(commands_open)
        (return_code
        ,return_stdo
        ,return_stde) = _popen(command_exec)
...
def _command_process(command_exec, server_queued_fifo=[]):
    ...
    process_run = multiprocessing.Process(target=_command, args=(command_exec, process_ret))
...
def _clear(self=None):
    if   _search('nt|ce|java'):
        _system('cls')
        print('\r', end='')
    elif _search('posix|riscos|os2'):
        _system('clear')